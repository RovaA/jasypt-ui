 ...
 * @author andri
 */
public class MultiEncryptionTabController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        passwordTextField.textProperty().addListener((obs, oldValue, newValue) -> encryptAllRows());
        algorithmChoiceBox.valueProperty().addListener((obs, oldValue, newValue) -> encryptAllRows());
    ...
    @FXML
    private void onEncryptAll(ActionEvent event) {
        encryptAllRows();
    }
    ...
    private void addFieldRow() {
        ...
        valueField.setPromptText("Value to encrypt");
        ...
        valueField.textProperty().addListener((obs, oldValue, newValue) -> encryptRow(row));

        fieldsContainer.getChildren().add(row);
    }

    private void encryptAllRows() {
        fieldsContainer.getChildren().forEach(this::encryptRow);
    }

    private void encryptRow(Node node) {
        var row = (HBox) node;
        ...
        try {
            ...
            resultField.setText(jasyptService.encrypt(data));
        } catch (Exception e) {
            resultField.clear();
        }
    }
